...
void Vulkan::createInstance()
{
	...
	if (vkCreateInstance(&createInfo, nullptr, &m_instance) != VK_SUCCESS)
		throw std::runtime_error("Erreur : Cr�ation de l'instance !");
}
...
void Vulkan::createSwapChain()
{
	...
	for (uint32_t i = 0; i < m_swapChainImages.size(); i++)
		m_swapChainImageViews[i] = createImageView(m_swapChainImages[i], m_swapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1, VK_IMAGE_VIEW_TYPE_2D);
}
...
void Vulkan::createSwapchainFramebuffers(VkRenderPass renderPass, VkSampleCountFlagBits msaaSamples, VkImageView colorImageView)
{
...
void Vulkan::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels, uint32_t arrayLayers)
{
...
VkImageView Vulkan::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels, VkImageViewType viewType)
{
...
VkFormat Vulkan::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
{
	...
	throw std::runtime_error("Aucun format support� !");
}
...
void Vulkan::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer & buffer, VkDeviceMemory & bufferMemory)
{
	...
	if (vkCreateBuffer(m_device, &bufferInfo, nullptr, &buffer) != VK_SUCCESS)
		throw std::runtime_error("Erreur : cr�ation d'un buffer");
	...
	if (vkAllocateMemory(m_device, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS)
		throw std::runtime_error("Erreur : allocation de m�moire");
...
void Vulkan::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height, uint32_t baseArrayLayer)
{
	VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void Vulkan::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels, uint32_t baseArrayLayer)
{
	...
	if (!(formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT)) {
		throw std::runtime_error("[Mipmap generation] Format non supported");
	}
			...
			vkCmdBeginRenderPass(m_commandBuffersSwapChain[i], &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
				...
				for (int j = 0; j < meshes.size(); ++j)
				{
					for (int k(0); k < meshes[j].vertexBuffer.size(); ++k)
					{
						...
						if (meshes[j].instanceBuffer.size() > 0)
						{
							VkBuffer instanceBuffers[] = { meshes[j].instanceBuffer[k] };
							vkCmdBindVertexBuffers(m_commandBuffersSwapChain[i], 1, 1, instanceBuffers, offsets);
						}
